...
export default function ImgConvert() {
  ...
  const run = async () => {
    ...
    try {
      ...
      downloadBlob(blob, replaceExt(files[0].name, format), IMAGE_MIME[format])
      addHistory({ toolSlug: 'img-convert', toolName: '轉換圖片格式', fileName: files[0].name, result: format.toUpperCase(), ok: true })
    } catch (e) {
  ...
  return (
    <ToolFrame title="轉換圖片格式" subtitle="在 PNG、JPEG、WebP 之間互相轉換。">
      <Dropzone accept="image/*" kind="image" files={files} onFiles={setFiles} hint="支援 PNG / JPEG / WebP / GIF / BMP" />
      {files[0] && (
      ...
      <div className="flex justify-center">
        <Button onClick={run} disabled={!files[0] || busy} icon="transform">
          {busy ? '轉換中…' : `轉換為 ${format.toUpperCase()}`}
        </Button>
      </div>
    </ToolFrame>
  )
